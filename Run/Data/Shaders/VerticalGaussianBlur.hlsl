//----------------------------------------------------------------
Texture2D screenColor : register(t0);
Texture2D originalScreenColor : register(t1);
Texture2D depthTexture : register(t2);
SamplerState screenSampler : register(s0);

//----------------------------------------------------------------
struct vs_input_t
{
    float3 modelSpacePosition : POSITION;
    float4 color : COLOR;
    float2 uv : TEXCOORD;
};

//----------------------------------------------------------------
struct v2p_t
{
    float4 clipSpacePosition : SV_POSITION;
    float4 color : COLOR;
    float2 uv : TEXCOORD;
};

//----------------------------------------------------------------
v2p_t VertexMain(vs_input_t input)
{
    v2p_t v2p;
    v2p.clipSpacePosition = float4(input.modelSpacePosition, 1.f);
    v2p.color = input.color;
    v2p.uv = input.uv;
    return v2p;
}

//----------------------------------------------------------------
float4 PixelMain(v2p_t input) : SV_Target0
{
    float2 texelSize = float2(0.0f, 1.0f / 600.0f);
    float4 color = float4(0, 0, 0, 0);
    
    color += screenColor.Sample(screenSampler, input.uv - texelSize * 3.23076923f) * 0.07027027f;
    color += screenColor.Sample(screenSampler, input.uv - texelSize * 1.38461538f) * 0.31621622f;
    color += screenColor.Sample(screenSampler, input.uv) * 0.22702703f;
    color += screenColor.Sample(screenSampler, input.uv + texelSize * 1.38461538f) * 0.31621622f;
    color += screenColor.Sample(screenSampler, input.uv + texelSize * 3.23076923f) * 0.07027027f;
    
    return color;
}