...
float4 PixelMain(v2p_t input) : SV_Target0
{
    ...
    float4 texturePointColor = diffuseTexture.Sample(pointSampler, input.uv);
    float4 textureAnisoColor = diffuseTexture.Sample(anisoSampler, input.uv);
    float2 dx = ddx(input.uv);
    float2 dy = ddy(input.uv);
    float delta = max(dot(dx, dx), dot(dy, dy));
    float blendFactor = smoothstep(0.00001, 0.0000675, delta);
    float4 textureColor = lerp(texturePointColor, textureAnisoColor, blendFactor);
